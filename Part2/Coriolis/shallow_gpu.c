...
inline double get_value(const struct data *data, int i ,int j){
    return data->values[(data->nx * j) + i];}

inline void set_value(struct data *data, int i, int j , double val){
...
int main(int argc, char **argv)
{
  ...
  double c1_dx = c1 * invdx;
  double c1_dy = c1 * invdy;

  //Force Coriolis 
  double f_coriolis = 3.78e-5;
  ...
  #pragma omp target teams distribute parallel for collapse(2) \
    map(to: h, h.values[0:h.nx*h.ny]) \
    map(to: invdx, invdy, param.dx, param.dy) \
    map(to: h_u) \
    map(tofrom: h_u.values[0:h_u.nx*h_u.ny])
  for(int j = 0; j < ny; j++) 
  {
...
#pragma omp target teams distribute parallel for collapse(2) \
    ...
    map(to: invdx, invdy, param.dx, param.dy) \
    map(to: h_v) \
    map(tofrom: h_v.values[0:h_v.nx*h_v.ny])
...
#pragma omp target enter data map(to: eta, eta.values[0:eta.nx*eta.ny])
#pragma omp target enter data map(to: u, u.values[0:u.nx*u.ny])
#pragma omp target enter data map(to: v, v.values[0:v.nx*v.ny])
#pragma omp target enter data map(to: h_u, h_u.values[0:h_u.nx*h_u.ny])
#pragma omp target enter data map(to: h_v, h_v.values[0:h_v.nx*h_v.ny])
...
#pragma omp target teams distribute parallel for map(to:sin_t,A)
      for(int i = 0; i < nx; i++) 
      {
        set_value(&v, i, 0, 0.);
        set_value(&v, i, nx, A * sin_t);
      }
    }
    ...
    // update eta
#pragma omp target teams distribute parallel for collapse(2) map(to :param.dt,invdx,invdy)
    for(int j = 0; j < ny ; j++) 
    {
      for(int i = 0; i < nx; i++) 
      {
    ...
    // update u and v
#pragma omp target teams distribute parallel for collapse(2) map(to :c1_dx,c1_dy,c2)
    for(int j = 0; j < ny; j++) 
    {
      for(int i = 0; i < nx; i++) 
      {
        ...
        double old_u = get_value(&u, i, j);
        double old_v = get_value(&v, i, j);
        
        double u_ij = (1. - c2) * old_u - (c1_dx) * (eta_ij - eta_imj) + (param.dt * f_coriolis * old_v);
        double v_ij = (1. - c2) * old_v - (c1_dy) * (eta_ij - eta_ijm) - (param.dt * f_coriolis * old_u);
        set_value(&u, i, j, u_ij);
        set_value(&v, i, j, v_ij);
      }
    }
  if(param.source_type == 3)
      {
        #pragma omp target teams distribute parallel for collapse(1) map(to :param.dt,invdx,invdy,ny,nx)
        for(int i = 0; i<nx; i++)
        {
          double top = get_value(&v, i, ny);
          double bot = get_value(&v, i, 0);
          set_value(&v, i, ny, top - (param.dt * invdy * (sqrt(param.g * get_value(&h_v, i, ny)) * (top - get_value(&v, i, ny - 1)))));
          set_value(&v, i, 0, bot - (param.dt * invdy * (sqrt(param.g * get_value(&h_v, i, 0)) * (bot- get_value(&v, i, 1)))));
        }
        #pragma omp target teams distribute parallel for collapse(1) map(to :param.dt,invdx,invdy,ny,nx)
        for(int j = 0; j < ny; j++)
        {
          double left = get_value(&u, 0, j);
          double right = get_value(&u, nx, j);
          set_value(&u, 0, j, left - (param.dt * invdx * (sqrt(param.g * get_value(&h_u, 0, j)) * (left - get_value(&u, 1, j)))));
          set_value(&u, nx, j, right - (param.dt * invdx * (sqrt(param.g * get_value(&h_u, nx, j)) * (right - get_value(&u, nx - 1, j)))));
  ...
  //                   param.dt, nt, param.sampling_rate);
    ...
    #pragma omp target exit data map(delete: eta, eta.values[0:eta.nx*eta.ny])
    #pragma omp target exit data map(delete: u, u.values[0:u.nx*u.ny])
    #pragma omp target exit data map(delete: v, v.values[0:v.nx*v.ny])
    #pragma omp target exit data map(delete: h_u, h_u.values[0:h_u.nx*h_u.ny])
    #pragma omp target exit data map(delete: h_v, h_v.values[0:h_v.nx*h_v.ny])
    free_all(&eta, &u, &v, &h_u, &h_v,&h);
