...
int main(int argc, char **argv)
{
    ...
    for(int n = 0; n < nt; n++) 
    {
        ...
        if(param.source_type == 3)
        {
            ...
            for(int i = 0; i<sub_nx; i++) 
            {
                double top = get_value(&v, i, sub_ny);
                double bot = get_value(&v, i, 0);
                if(rank / Px == Py - 1) 
                {
                    set_value(&v, i, sub_ny, top - (param.dt * invdy* sqrt(param.g * get_value(&h_v, i, sub_ny)) * (top - get_value(&v, i, sub_ny - 1))));
                }
                if(rank / Px == 0) 
                {
            ...
            for(int j = 0; j < sub_ny; j++) 
            {
                ...
                double right = get_value(&u, sub_nx, j);
                if(rank % Px == 0) 
                {
                ...
                if(rank % Px == Px - 1) 
                {
                    set_value(&u, sub_nx, j, right - (param.dt * invdx *sqrt(param.g * get_value(&h_u, sub_nx, j)) * (right - get_value(&u, sub_nx - 1, j))));
                }
            }
        }
