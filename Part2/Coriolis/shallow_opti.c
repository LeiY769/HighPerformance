...
void apply_absorbing_boundaries_eta(struct data *eta, double c, double dt, double dx, double dy) {
    double factor_x = c * dt / dx;
    double factor_y = c * dt / dy;
    
    // Appliquer aux bords gauche et droit
    for (int j = 0; j < eta->ny; j++) {
        // Bord gauche
        double inner_val = get_value(eta, 1, j);
        double boundary_val = get_value(eta, 0, j);
        double new_val = boundary_val - factor_x * (boundary_val - inner_val);
        set_value(eta, 0, j, new_val);
        
        // Bord droit
        inner_val = get_value(eta, eta->nx - 2, j);
        boundary_val = get_value(eta, eta->nx - 1, j);
        new_val = boundary_val - factor_x * (boundary_val - inner_val);
        set_value(eta, eta->nx - 1, j, new_val);
    }
    
    // Appliquer aux bords haut et bas
    for (int i = 0; i < eta->nx; i++) {
        // Bord bas
        double inner_val = get_value(eta, i, 1);
        double boundary_val = get_value(eta, i, 0);
        double new_val = boundary_val - factor_y * (boundary_val - inner_val);
        set_value(eta, i, 0, new_val);
        
        // Bord haut
        inner_val = get_value(eta, i, eta->ny - 2);
        boundary_val = get_value(eta, i, eta->ny - 1);
        new_val = boundary_val - factor_y * (boundary_val - inner_val);
        set_value(eta, i, eta->ny - 1, new_val);
    }
}

void apply_absorbing_boundaries_u(struct data *u, double c, double dt, double dx, double dy) {
    double factor_x = c * dt / dx;
    double factor_y = c * dt / dy;
    ...
    for (int j = 0; j < u->ny; j++) {
        // Bord gauche
        double inner_val = get_value(u, 1, j);
        double boundary_val = get_value(u, 0, j);
        double new_val = boundary_val - factor_x * (boundary_val - inner_val);
        set_value(u, 0, j, new_val);
        
        // Bord droit
        inner_val = get_value(u, u->nx - 2, j);
        boundary_val = get_value(u, u->nx - 1, j);
        new_val = boundary_val - factor_x * (boundary_val - inner_val);
        set_value(u, u->nx - 1, j, new_val);
    }
    
    // Appliquer aux bords haut et bas
    for (int i = 0; i < u->nx; i++) {
        // Bord bas
        double inner_val = get_value(u, i, 1);
        double boundary_val = get_value(u, i, 0);
        double new_val = boundary_val - factor_y * (boundary_val - inner_val);
        set_value(u, i, 0, new_val);
        
        // Bord haut
        inner_val = get_value(u, i, u->ny - 2);
        boundary_val = get_value(u, i, u->ny - 1);
        new_val = boundary_val - factor_y * (boundary_val - inner_val);
        set_value(u, i, u->ny - 1, new_val);
    }
}
  ...
  for(int n = 0; n < nt; n++) 
  {
    ...
    for(int j = 0; j < ny; j++) 
    {
      for(int i = 0; i < nx; i++) 
      {
        ...
        double u_ij = (1. - c2) * old_u
          - (c1_dx) * (eta_ij - eta_imj) + (param.dt * f_coriolis * old_v);
        double v_ij = (1. - c2) * old_v
          - (c1_dy) * (eta_ij - eta_ijm) - (param.dt * f_coriolis * old_u);
