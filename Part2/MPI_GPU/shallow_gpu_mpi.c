...
inline double *top_row(struct data *data){
    return &(data->values[0]);}

inline double *internal_first_row(struct data *data){
    return &(data->values[data->nx]);}

inline double *internal_last_row(struct data *data){
    return &(data->values[(data->ny - 2) * data->nx]);
}
inline double *bottom_row(struct data *data){
    return &(data->values[(data->ny - 1) * data->nx]);}

inline double *left_col(struct data *data, int j){
    return &(data->values[j * data->nx]);}

inline double *internal_first_col(struct data *data, int j){
    return &(data->values[j * data->nx + 1]);}

inline double *internal_last_col(struct data *data, int j){
    return &(data->values[j * data->nx + data->nx - 2]);}

inline double *right_col(struct data *data, int j){
...
#pragma omp target teams distribute parallel for 
      ...
      {
        set_value(&u, 0, j, 0.);
        set_value(&u, sub_ny, j, 0.);

      }
#pragma omp target teams distribute parallel for map(to:v_speed)
      for(int i = 0; i < sub_nx; i++) 
      {
        set_value(&v, i, 0, 0.);
        set_value(&v, i, sub_nx, v_speed);
      }
    }
...
#pragma omp target teams distribute parallel for map(to:sin_t,A)
      set_value(&eta, sub_nx / 2, sub_ny / 2, A * sin_t);
    }
    else 
    {
...
#pragma omp target teams distribute parallel for collapse(2) map(to :c1_dx,c1_dy,c2)
    ...
    #pragma omp target update to(u.values [0:u.nx * u.ny])
    #pragma omp target update to(v.values [0:v.nx * v.ny])

    MPI_Barrier(MPI_COMM_WORLD);
  }

  write_manifest_vtk(param.output_eta_filename,
                     param.dt, nt, param.sampling_rate, world_size);
