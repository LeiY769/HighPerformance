...
int main(int argc, char **argv)
{
  ...
  // Variable declaration
   ...
   const double alpha = 2* M_PI *f;

  // Constant factor to get the multiplication instead of division
  const double invdx = 1./param.dx;
  const double invdy = 1./param.dy;
  const double c1_dx = c1 * invdx;
  const double c1_dy = c1 * invdy;
  ...
  for (int i = 0; i < 10 && i < h.nx * h.ny; i++) {
    printf("h[%d] = %f\n", i, h.values[i]);
  }
...
//print to see
  printf("h_u initialization complete.\n");
  for (int i = 0; i < 10 && i < h_u.nx * h_u.ny; i++) {
    printf("h_u[%d] = %f\n", i, h_u.values[i]);
  }
  printf("h_v initialization complete.\n");
  for (int i = 0; i < 10 && i < h_v.nx * h_v.ny; i++) {
    printf("h_v[%d] = %f\n", i, h_v.values[i]);
  }
  // Copy data to GPU
...
#pragma omp target teams distribute parallel for map(to:sin_t,A,ny)
      for(int i = 0; i < nx; i++) 
      {
        set_value(&v, i, 0, 0.);
